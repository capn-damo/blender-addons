...
def run_resync(self,context):
    ...
    if current_img=="":
        print("Using a default active background")
        self.report({'INFO'}, "Using a default active background")
...
def node_resync(self,context):
    ...
    if settings.use_advanced_sky == True and len(easyhdr_g.inputs)<5:
        ...
        # apply the settings even though should stay false, better ultimately
...
class resync_active(bpy.types.Operator):
    """Resync the active background"""
    bl_idname = "scene.prolightingskies_resync"
    bl_label = "Resync the active background"
    bl_description = "After loading a file previously using backgrounds, "+\
    ...
    for g_n in g_nodes:
        ...
        for prop_id, prop in g_n.bl_rna.properties.items():
            ...
            if prop_id == "node_tree":
                ...
                # so far so fine, because there is only one internal group.
    ...
    for label, desc, img_name, sun, sky in environments:
        if img_name == enum_choice:
            ...
            if settings.use_advanced_sky==True:
                ...
                apply_background_color_change( [tmp.r,tmp.g,tmp.b,1], easyhdr_g)
                apply_mirror_sky_change(settings.mirror_sky, easyhdr_g)
                apply_change_horizon(settings.horizon, easyhdr_g)
...
def apply_change_node_image(tex_node, image_name, image_source_path):
    ...
    # if not, the user has deleted or switched the image, we create a new one
        ...
        if bpy.data.use_autopack:
            # autopack prevents updating images, re-import after removing users
            try:
                # tex_node.image
...
def apply_image_change(enum_choice, g_nodes):
    ...
    if os.path.isfile(path_img_exr)== False and\
        ...
        x = "uhoh"
        print("Could not find background files, check HDRs are installed correctly")
...
def set_image_by_category(new_choice):
    ...
    if new_choice not in env_names:
        ...
        print("Error, background not found in this category: {x}, category {y}"\
                .format(x=new_choice,y=category_selected))
        global newload
        newload = False # clear it out
        
        if len(sublist[0][2])>0:
            print("Setting to first background")
...
def category_items(self, context):
    ...
    items = []
    items.append(("all","All backgrounds","Show all backgrounds"))
    sorted_cats = sorted(categories)
    for item in sorted_cats:
        items.append((item, item, "Show all backgrounds in the '"+item+"' category"))
    return items
...
class nextSky(bpy.types.Operator):
    """Choose the next background available"""
    bl_idname = "scene.prolightingskies_next"
    bl_label = "Choose the next background available"
...
class previousSky(bpy.types.Operator):
    """Choose the previous background available"""
    bl_idname = "scene.prolightingskies_previous"
    bl_label = "Choose the previous background available"
...
class install_skies_from_zip_delay(bpy.types.Operator):
    ...
    def start(self, context, filepath):
        ...
        reload_all_JSON()
        self.report({'INFO'}, "{x} backgrounds installed".format(x=numInstalled))
...
class fav_toggle_op(bpy.types.Operator):
    """Set or remove this background from favorites"""
    bl_idname = "scene.proskies_fave_toggle_op"
    bl_label = "Toggle Favorite"
    bl_description = "Toggle the favorite status of this background"
    ...
    def execute(self,context):
        ...
        if type(data['categories']['favorites']) != type({}):
            self.report({'ERROR'},"Error processing favorites in envs.json,"+\
                    " please reinstall Pro Lighting: Backgrounds")
            return {'CANCELLED'}
        ...
        # ensure categories/etc are properly updated
        environments = data["environments"]
        categories = data["categories"]
...
class reloadJSONS(bpy.types.Operator):
    """Reload all backgrounds/icons, use after manually copying HDRs to the hdris folder"""
    bl_idname = "scene.prolightingskies_reload_jsons"
    bl_label = "Reload all backgrounds/icons, use after manually copying HDRs to the hdris folder"
...
class openHDRFolder(bpy.types.Operator):
    """Open the hdr folder for manual background installing"""
...
class CyclesWorld_PT_ProLightingSkies(Panel):
    """Pro Lighting: Backgrounds"""
    bl_idname = "CyclesWorld_PT_ProLightingSkies"
    bl_label = "Pro Lighting: Backgrounds"
    ...
    def draw(self, context):
        ...
        if use_icons:
            ...
            if newload == False:
                if hasattr(world,"env_previews_"+category_selected)==True:
                    ...
                    # default category, or one from previous version
        ...
        if settings.show_advanced == False:
            ...
            row = layout.row()
            col = row.column()
            col.operator("scene.prolightingskies_reset")
            col = row.column()
            if newload==True:
                col.enabled=False
            col.prop(settings,"use_advanced_sky")
            
            box = layout.box()
            box.enabled = settings.use_advanced_sky
            ...
            row = layout.row()
            scene = context.scene
            cscene = scene.cycles # see if we can use cworld instead
            split = row.split()
            col = split.column()
...
class ProLightingSkiesSettings(PropertyGroup):
    ...
    use_pl_skies = BoolProperty(
        name="Pro Lighting: Backgrounds",
        description="Use a Pro Lighting: Backgrounds node setup for the world",
        default=False,
    ...
    z_rotation = FloatProperty(
        name="Rotation",
        description="Rotation of the Background around Z",
    ...
    mirror_sky = FloatProperty(
        name="Mirror sky",
        description="Mix between no and full background top/bottom mirroring",
    ...
    show_advanced = BoolProperty(
        ...
        description="Show/hide the advanced background lighting options",
        default=False,
    )
    ...
    current_favorite = BoolProperty(
        ...
        description="Set or remove this background from favorites",
        default=False,
    )
    
    category = EnumProperty(
        name="Background category",
        description="Category of backgrounds to show",
        update=category_load,
        items=category_items
    )
    category_string = bpy.props.StringProperty(
        name="Internal String Name",
        description="String category of backgrounds to show, for resync",
        default=""
    )
    image_string = bpy.props.StringProperty(
        name="Internal image Name",
        description="String image of backgrounds to show, for resync",
        default=""
    )
...
class prolightingSkiesPreferencePanel(bpy.types.AddonPreferences):
    ...
    show_custom_install = bpy.props.BoolProperty(
        ...
        description = "Instructions for installing custom sets of backgrounds",
        default = False
    )
    builtin_counted = IntProperty(
        name = "Builtin backgrounds counted",
        description = "Backend property for verifying proper count"+\
            "of provided backgrounds installed",
        default = 0
    )
    ...
    def draw(self, context):
        ...
        if not_installed == True:
            ...
            #layout.label("All {x} of {y} HDRs installed. ".format( \
            #    x=self.installedHDRcount,y=len(environments)), icon="FILE_TICK")
        ...
        box.label(text="2. Locate the HDR zip you downloaded from Blender Guru")
        ...
        col.operator("scene.prolightingskies_reload_jsons", \
            text="Reload all backgrounds", icon="LOOP_BACK")
        col = row.column()
...
def register():
    ...
    if not load_handler in bpy.app.handlers.load_post:
